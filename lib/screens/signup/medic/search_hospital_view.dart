...
import 'package:medic_app/services/api/lambda/hospital_service.dart';
import 'package:medic_app/widgets/hospital_card.dart';
...
class SearchHospitalView extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _SearchHospitalView();
}

class _SearchHospitalView extends State<SearchHospitalView> {
  late TextEditingController _searchTextEditingController;
  String _searchText = '';
  int _searchPage = 0;

  ValueNotifier<List<HospitalData>> _addressList =
      ValueNotifier<List<HospitalData>>([]);
  ...
  Future<void> _getAddressList() async {
    ...
    HospitalService().getHospital(_searchText).then((value) {
      if (value.containsKey('err')) {
        ...
        _addressList.value
            .insertAll(_addressList.value.length, value['data'].hospitals);
        setState(() {});
      }
    ...
    return ValueListenableBuilder(
        ...
        builder: (BuildContext context, bool isInit, _) {
          return ValueListenableBuilder(
              ...
              builder: (BuildContext context, List<HospitalData> list, _) {
                return isInit
                    ? list.isNotEmpty
                        ? Column(
